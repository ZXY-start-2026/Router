...
> 迭代同步状态（2026-07-24）：迭代1～5均已完成实施。迭代3、4、5实现记录分别见 `docs/iteration3-plan.md`、`docs/iteration4-plan.md`、`docs/iteration5-plan.md`。当前后端 50 项、前端 21 项测试、TypeScript 检查、Vite 生产构建、Alembic `0003 ↔ 0005` 升降级和 Playwright Chromium 核心流程均已通过。

> 文档状态：迭代1～5需求、设计与实现已同步
> 目标环境：Windows 本地开发  
...
当前工程已经按本文顺序完成迭代1～5；下表保留各迭代边界，作为历史验收和后续维护依据：
...
12. 模型协议固定：模型请求只发送 `prompt`、`max_tokens`、`temperature`、`stop`，不发送 API Key、`Authorization`、`logprobs` 或 `logprobs_mode`；`stop` 使用 `["\nUser:", "\nSystem:"]` 防止模型继续生成下一轮；响应必须按已确认的 text completion 结构解析。
...
以下目录展示迭代1～5完成后的模块位置。少量测试名称保留为职责级架构位置；当前实际文件以仓库为准。后续迭代仍需在目录树中标明计划位置，但不得提前创建无功能占位文件。
...
backend/
  ...
  tests/
    ...
    unit/
      ...
      test_title.py
      test_routing.py
      test_router_assets.py
      test_prompt_rendering.py
      test_completion_provider.py
      test_costs.py
...
所有聊天回答模型收到完全相同的文本结构；不发送 `messages` 数组。空的角色、备忘录或搜索段不输出空标题。`System:` 段内部固定按系统规则、角色、系统摘要、用户保护区、搜索上下文排列；随后按分支逻辑位置输出尚未被摘要覆盖的历史有效轮，最后输出当前问题并以 `Assistant:` 结尾。靠后的内容优先级更高，因此整体冲突优先级为“当前消息 > 最近原始对话 > 用户保护区 > 系统摘要”：
...
内部 `[MEMORY_TASK]` 不属于聊天回答，不进入 ContextSnapshot。Provider 只在该标记下按固定模型适配指令模板：Gemma 使用 `<start_of_turn>`，配置 `disable_thinking=true` 的模型使用 `User:/Assistant:` 和 `/no_think`。这一适配用于保证结构化 JSON，不改变普通聊天的统一 Prompt。
...
sequenceDiagram
    ...
    UI->>API: "PUT /conversations/{id}/role(RoleContentRequest)"
    API->>RS: "update(conversation_id, content)"
    RS->>RR: "create_version(conversation_id, normalized_content)"
    RR->>DB: "新增不可变 RoleVersion"
    RS->>RR: "set_active_for_branch(active_branch, version)"
    RR->>DB: "更新 active_role_version_id"
    API-->>UI: "CurrentRoleResponse"
    UI->>API: "POST 下一条消息"
    API->>CS: "prepare(branch_id, message, search_snapshot)"
    CS->>RR: "get_current_for_branch(branch_id)"
    RR-->>CS: "RoleVersion 或空"
    CS->>DB: "保存 role_version_id 与 role_text 快照"
```

停用角色调用 `POST /conversations/{id}/role/deactivate`，只清空活动分支指针。回答分支优先继承目标回答 `ContextSnapshot.role_version_id`，编辑消息分支继承原消息位置实际使用的角色；缺失目标上下文时回退到分叉前最近上下文，禁止使用源分支当前角色追溯填充历史。
...
- 左侧会话“删除”是浏览器本地隐藏，不调用后端；后端保留的 `DELETE /conversations/{id}` 仅执行 `ARCHIVED` 软归档，绝不物理删除数据。
...
| 1 | `DELETE /api/v1/conversations/{id}` | 后端软归档；当前前端隐藏入口不调用 |
...
| 3 | `GET /api/v1/messages/{id}/answers?branch_id=` | 按需读取成功回答版本 |
...
| 5 | `POST /api/v1/conversations/{id}/role/deactivate` | 停用当前分支角色，不删除版本 |
...
`GET /api/v1/branches/{id}/memory` 返回当前版本、最近自动更新状态和只读全局 `n/k/m`。允许读取任意有效分支。

`GET /api/v1/branches/{id}/memory/versions?limit=20&cursor=` 按 `version_number DESC` 游标分页，只返回该分支版本；`is_current` 由 Branch 指针派生。
...
两种写操作只允许作用于会话当前活动分支。模型更新失败返回业务失败结果并保留原当前版本，不把已成功的聊天回答改为失败。

#### 3.8 角色与模板

`PUT /api/v1/conversations/{id}/role` 请求包含 `name`、`persona`、`background`、`domain`、`traits`、`style`、`constraints_text` 和可空 `source_template_id`。保存时创建不可变 `RoleVersion` 并只更新当前活动分支指针。

`POST /api/v1/conversations/{id}/role/deactivate` 无正文。已经无角色时幂等成功；操作只清空活动分支的 `active_role_version_id`，不删除角色版本或修改历史 `ContextSnapshot`。

`POST /api/v1/role-templates` 创建不可变模板，不自动应用到当前分支。选择模板只发生在前端表单，只有再次保存角色时才创建 `RoleVersion`。
...
| `active_memory_version_id` | UUID/null | FK MemoryVersion，当前分支的备忘录指针 |
| `active_role_version_id` | UUID/null | FK RoleVersion；角色版本属于会话，生效指针属于分支 |
...
`RoleTemplate`：`id`、`name`、`persona`、`background`、`domain`、`traits_json`、`style`、`constraints_text`、`created_at`。当前为单用户本地模板，不加 owner 字段。模板创建后不可编辑或删除，只用于填充角色表单。

`RoleVersion`：`id`、`conversation_id`、`version_number`、`source_template_id`、`name`、`persona`、`background`、`domain`、`traits_json`、`style`、`constraints_text`、`created_at`。唯一约束为 `(conversation_id, version_number)`。角色内容创建后不可更新；分支通过 `active_role_version_id` 决定后续消息所用版本，停用角色只清空该指针。
...
erDiagram
    ...
    BRANCH }o--o| MEMORY_VERSION : selects_current
    ...
    BRANCH }o--o| ROLE_VERSION : selects_current
...
| `memory_model_id` | `MEMORY_MODEL_ID` | `MODEL_A` | 固定执行摘要和冲突检测，必须引用已启用的 `MODEL_A/B/C` |
| `system_rules_text` | `SYSTEM_RULES_TEXT` | 配置文件提供 | 进入每次上下文并写入快照 |

`MEMORY_N/K/M` 仅为应用级配置，不在 Conversation 保存覆盖值。备忘录模型不经过路由、不降级；暂时性错误只用同一固定模型重试一次。普通自动生成仍按“同模型暂时性错误最多重试一次，再按不可变排名降级”执行。模型请求超时由每个模型的 `request_timeout_seconds` 明确配置。
...
| `disable_thinking` | boolean | 对需要显式关闭思考的模型在 Prompt 中加入 `/no_think`；默认 false |
...
以下描述覆盖迭代1～5的现有代码职责。配置、迁移和测试文件在本节末单独说明。
...
a. 文件用途说明：迭代5定义角色、停用结果与模板输入输出。

b. 文件内类图：`RoleContentRequest`、`RoleVersionResponse`、`CurrentRoleResponse`、`RoleTemplateRequest`、`RoleTemplateResponse`、`RoleTemplateListResponse`。

c. 函数/方法详解：名称去除首尾空白后必须非空；`traits` 去除空项并按输入顺序去重；`domain` 作为独立专业领域字段保存。客户端不能提交渲染后的 system prompt。
...
b. 文件内类图：`RoleRepository` 提供 `list_templates`、`get_template`、`create_template`、`get_version`、`next_version_number`、`create_version`、`get_current_for_branch`、`set_active_for_branch`、`clear_active_for_branch`。

c. 函数/方法详解：各方法输入会话/分支与角色内容，输出对应 ORM；设置分支指针时必须验证 RoleVersion 属于同一 Conversation。Repository 不提供模板或角色版本的更新、删除方法。
...
classDiagram
    class MemoryService {
      ...
      -summarize(current_summary, protected_text, new_turns, recent_turns)
      -parse_model_result(content, fallback_summary, checked_position)
    }
```
...
- 用途：选择覆盖范围严格早于分叉位置的最新源版本，并在新分支创建 `BRANCH_INHERIT` 本地版本。
- 输入参数：源分支、新分支、分叉逻辑位置。
- 输出数据结构：新分支 MemoryVersion 或空；源分支版本和指针不改变。
...
- `deactivate(conversation_id)`：幂等清空当前活动分支角色指针，不删除版本。
- `create_template(content)` / `list_templates()`：新增或读取本地模板，不更新已有版本。
- `render(role_version)`：按名称、人格、背景、专业领域、特征、风格、约束固定顺序转为上下文文本，跳过空段。
- `resolve_at_fork(...)`：从分叉位置实际使用的 ContextSnapshot 解析角色版本；没有目标上下文时只向前回退，不读取源分支当前指针。
...
a. 文件用途说明：统一管理活动会话消息、分支、回答版本缓存、备忘录、角色模板、同步操作状态和刷新。
...
- 用途：封装聊天区数据和所有动作。
- 输入参数：可空当前会话 ID。
- 输出数据结构：除消息与分支状态外，包含 `memory/memoryVersions/currentRole/roleTemplates`，以及 `send/regenerate/activateAnswer/editMessage/switchBranch/loadMemoryVersions/saveProtectedMemory/restoreMemory/loadRoleTemplates/saveRole/deactivateRole/createRoleTemplate/reload`。
...
a. 文件用途说明：展示顶部会话信息、分支消息列表、空/失败状态和输入区，并协调备忘录、角色两个独立右侧抽屉。

b. 文件内类图：函数组件 `ChatPanel`。

c. 函数/方法详解：`ChatPanel(props)` 输入当前会话、消息、分支、备忘录、角色及动作；输出主聊天区域。连接状态、备忘录/角色入口和分支工具栏相互独立；单分支不显示切换器。发送期间显示同步等待态，但不伪装流式 Token。
...
a. 文件用途说明：为备忘录和角色提供共用右侧抽屉，不引入第三方 UI 依赖。

b. 文件内类图：函数组件 `SideDrawer`。

c. 函数/方法详解：`SideDrawer(props)` 输入 `open/title/onClose/children`；打开时渲染带 `role="dialog"` 的抽屉和遮罩，支持关闭按钮、Escape 和点击遮罩关闭，不修改面板业务状态。
...
- `handleRestore(versionId)`：二次确认后调用恢复接口；恢复只创建新版本，不直接激活旧记录。
- 历史版本只在用户展开后按需加载；系统摘要只读，冲突和最近更新失败使用独立提示展示。

### frontend/src/components/RolePanel.tsx

a. 文件用途说明：迭代5编辑名称、人格、背景、专业领域、特征、风格、约束并从模板填充。

b. 文件内类图：函数组件 `RolePanel`。

c. 函数/方法详解：`RolePanel(props)` 输入当前角色、模板和保存/停用动作；输出表单。选择模板只填充表单，保存时才创建 RoleVersion；保存模板不自动应用角色；停用前二次确认且失败时保留表单输入。

### frontend/src/components/panels.css

a. 文件用途说明：集中维护 SideDrawer、MemoryPanel、RolePanel 的抽屉、表单、版本与告警样式，避免继续扩大聊天操作样式文件。
...
- `0004_memory.py`：创建备忘录版本和同步更新记录，为 Branch 增加 `active_memory_version_id`，为 ContextSnapshot 增加 `memory_version_id`。
- `0005_roles.py`：创建角色模板/版本，为 Branch 增加 `active_role_version_id`，为 ContextSnapshot 增加 `role_version_id`。
...
- 覆盖：POST 完整 endpoint URL、只有 prompt/max_tokens/temperature/stop、无 Authorization/logprobs 字段；按已提供样例解析 `choices[0].text`、model、id、finish_reason 与 usage；覆盖 HTTP 错误、超时、非 JSON、字段缺失、空正文和 Token 合计不一致。
...
- 覆盖：无角色响应、保存角色创建新版本、会话内版本号递增、模板创建不自动应用、模板来源记录、停用幂等以及分支角色隔离。
...
- 覆盖：消息与当前回答、搜索失败标记、发送禁用、请求结束选择器复位、Enter 发送、Shift+Enter 换行，以及备忘录/角色入口与分支工具栏独立布局。

#### frontend/src/test/MemoryPanel.test.tsx

- 覆盖：只提交用户保护区、系统摘要只读、历史按需加载和恢复操作回调。

#### frontend/src/test/RolePanel.test.tsx

- 覆盖：模板选择只填充表单、专业领域字段、保存角色和模板不自动应用。

#### frontend/e2e/core-chat.spec.ts

- 用途：迭代5启动隔离 SQLite、Mock Provider 和已构建前端，通过真实浏览器覆盖创建会话、设置角色、发送消息、展示回答、编辑保护区以及失败提示布局。
- 选择 Playwright 的原因：组件测试无法验证浏览器、前端网络层与 FastAPI/SQLite 的完整连通；当前只保留两个核心用例，不扩展为大型测试平台。
...
迭代2验收重点：每个新消息一个搜索快照；全部尝试逐字复用同一 Prompt；自动模式保存三个候选记录；MIRT accuracy、配置 estimated output Token 与成本字段来源正确；首选为最高综合得分合格模型；自动降级严格按快照；模型请求无鉴权且只有四个正式字段；响应 usage 与实际成本落库；手动模式无 RouteSnapshot 且不降级；页面完整显示搜索失败和生成元信息。
...
迭代4已经实施。备忘录以分支为所有者，以版本为唯一写入单位；`MEMORY_N/K/M` 只使用全局配置，不在 Conversation 保存覆盖字段。ContextService 把实际使用的版本 ID、保护区和系统摘要写入不可变快照。
...
“轮数”只统计该分支中有当前成功回答的 BranchMessage；遍历和摘要范围按 `logical_position` 保持原对话顺序。每次只处理一个计划批次：首次最多 `n-k` 轮，后续最多 `m` 轮。失败的备忘录尝试由 MemoryUpdateRecord 审计，下一次调用 `update_if_due` 时若积压多个批次，也只补偿最早一个批次，避免小模型一次复述大量原文。

恢复历史版本时不直接激活旧记录，因为旧版本可能没有覆盖现在已退出最近 k 轮窗口的有效历史。创建 RESTORE 新版本并增量补齐，才能同时满足“恢复基线”“最近原文保留”和“上下文不丢失”。

### 5. 迭代5演进路径

迭代5已经实施。角色内容归属 Conversation，版本不可变；每个 Branch 保存当前生效 RoleVersion 指针，以满足分叉点继承。修改角色只更新当前活动分支指针，旧 ContextSnapshot 仍记录旧版本，因此自然实现“仅后续消息”。切换到其他分支时展示并使用该分支自己的生效角色。

角色字段包含独立的 `domain` 专业领域；支持创建/选择模板和显式停用当前分支角色，不提供角色版本历史页面。前端复用右侧抽屉，并以 Playwright 覆盖核心 E2E。仍不加入多用户字段或权限框架；未来如果确有多用户需求，再通过单独迁移增加 owner 关系，不让当前本地系统承担无用复杂度。
...
迭代1～5均已完成。后续修改必须保持现有不可变历史、分支隔离、固定三模型路由、备忘录失败隔离和角色仅影响后续生成等约束。

当前三个模型已通过 `backend/config/models.yaml` 配置；360 搜索正式 Provider 资料仍未确认，因此继续保存明确的失败搜索快照并允许生成，不伪造搜索成功。后续发现新需求时，应先更新 `多模型路由聊天系统需求.md`、对应迭代文档与本 LLD，再修改代码，保持设计与实现同步演进。
